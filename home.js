/* ==== StepsAI landing page (index.html) redesign — interactive behaviour ==== */
...
/* Lenis — smooth, eased scrolling across the whole page (see reveal-on-scroll below for the per-element fade/slide) */
if (window.Lenis) {
      ...
      <div>
        <div class="browser-mock">
          ...
          <div class="browser-body">
            <div class="browser-hero" style="background:linear-gradient(rgba(15,23,42,.6),rgba(15,23,42,.75)),url('https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?auto=format&fit=crop&w=400&q=80') center/cover">
              <span class="bh-pill">NEW RELEASE</span>
              <h6>Autumn 2026 Trail Collection</h6>
              <p>Engineered for high peaks and heavy rain.</p>
            </div>
    ...
    <div class="ch-scene-grid">
      ...
      <div>
        <div class="browser-mock">
          ...
          <div class="browser-body">
            <div class="browser-hero" style="background:linear-gradient(rgba(15,23,42,.55),rgba(15,23,42,.75)),url('https://images.unsplash.com/photo-1600585154340-be6161a56a0c?auto=format&fit=crop&w=400&q=80') center/cover">
              <span class="bh-pill">NORTHGATE REALTY</span>
              <h6>Ask about any listing, anytime.</h6>
              <p>No app to download, no form to fill.</p>
            </div>
            <div class="browser-widget-pill">
              <span style="width:7px;height:7px;border-radius:50%;background:#4ade80"></span>
              StepsAI Active
...
/* ---------- Hero mockup: Sell more / Capture & book / Support better ---------- */
const HERO_TABS = {
  sales:`
    <div class="msg-row right"><span class="bub cust">I need a waterproof jacket for hiking. Which one should I get?</span><span class="avatar"><img src="https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=100&q=80" alt="Customer"></span></div>
    ...
    <div class="msg-row right"><span class="bub cust">Perfect, I'll take it!</span><span class="avatar"><img src="https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=100&q=80" alt="Customer"></span></div>`,
  book:`
    <div class="msg-row right"><span class="bub cust">Can I book a consultation with Dr. Mehta tomorrow?</span><span class="avatar"><img src="https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=100&q=80" alt="Patient"></span></div>
    <div class="msg-row">${BOT}<span class="bub ai">Of course! Here are available slots with Dr. Ananya Mehta:</span></div>
    <div class="doc-card" style="margin:4px 0 10px 39px;max-width:280px">
      <div class="doc-top">
    ...
    <div class="msg-row right"><span class="bub cust">10:30 AM works, thanks!</span><span class="avatar"><img src="https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=100&q=80" alt="Patient"></span></div>`,
  support:`
    <div class="msg-row right"><span class="bub cust">Where is my order #8472?</span><span class="avatar"><img src="https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&w=100&q=80" alt="Customer"></span></div>
    <div class="msg-row">${BOT}<span class="bub ai">It shipped yesterday via FedEx and arrives Thursday by 3:30 PM.</span></div>
    <div class="prod-card" style="margin:4px 0 10px 39px;max-width:280px">
      ...
      <div><h6>FedEx #FX-88391</h6><div class="pr" style="color:#16a34a;font-weight:700">Out for Delivery</div></div>
    </div>
    <div class="msg-row right"><span class="bub cust">Awesome, thank you!</span><span class="avatar"><img src="https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&w=100&q=80" alt="Customer"></span></div>`
};
const heroChatBody = document.getElementById('heroChatBody');
/* Measure every state up front and lock the panel to the tallest one, so
   switching tabs never jumps or resizes the card. */
(function normalizeHeroChatHeight(){
  if(!heroChatBody) return;
  const probe = heroChatBody.cloneNode(false);
  probe.style.cssText = 'position:absolute;visibility:hidden;pointer-events:none;height:auto;min-height:0';
  probe.style.width = heroChatBody.getBoundingClientRect().width + 'px';
  heroChatBody.parentNode.appendChild(probe);
  let max = 0;
  Object.values(HERO_TABS).forEach(html=>{ probe.innerHTML = html; max = Math.max(max, probe.scrollHeight); });
  probe.remove();
  if(max) heroChatBody.style.minHeight = max + 'px';
})();
document.querySelectorAll('.mini-feature[data-tab]').forEach(card=>{
  const select = () => {
    document.querySelectorAll('.mini-feature[data-tab]').forEach(c=>{ c.classList.remove('active'); c.setAttribute('aria-pressed','false'); });
    card.classList.add('active'); card.setAttribute('aria-pressed','true');
    if(heroChatBody){ heroChatBody.style.opacity=0; setTimeout(()=>{ heroChatBody.innerHTML = HERO_TABS[card.dataset.tab] || ''; heroChatBody.style.opacity=1; },160); }
  };
...
// Highlight top process step
function setWaActiveStep(stepNum){
  document.querySelectorAll('.was-step').forEach((s, idx)=>{
    s.classList.toggle('active-step', (idx + 1) === stepNum);
  });
}

// Main Interactive Pipeline Workflow Player
function playWaWorkflow(idx, triggerOrigin = 'click'){
  if(idx === 'custom' || idx >= WA_STUDIO_DATA.length) return;
  const d = WA_STUDIO_DATA[idx];
  if(!d) return;
  currentWaSegIdx = Number(idx);

  // Clear any existing timer
  if(waWorkflowTimer) clearTimeout(waWorkflowTimer);

  // 1. Stage 1: Audience Selection
  setWaActiveStep(1);
  document.querySelectorAll('#wacAudList .wac-aud-item').forEach((item, i)=>{
    const isSel = i === Number(idx);
    item.classList.toggle('active', isSel);
  });

  const selTag = document.getElementById('wacSelectedTag');
  if(selTag) selTag.innerHTML = '<span class="badge-dot"></span>' + d.selectedPill;
  const delCnt = document.getElementById('waDeliveredCount');
  if(delCnt) delCnt.textContent = d.deliveredCount;

  // 2. Stage 2 (t = 200ms): Composer Populate
  setTimeout(()=>{
    setWaActiveStep(2);
    const toPill = document.getElementById('wacToPill');
    if(toPill) toPill.textContent = d.toPill;
    const edBox = document.getElementById('wacEditorBox');
    if(edBox) edBox.innerHTML = d.editorHtml;
    const mImg = document.getElementById('wacMediaImg');
    if(mImg) {
      mImg.style.transition = 'opacity .18s ease';
      mImg.style.opacity = '0.3';
      setTimeout(()=>{ mImg.src = d.mediaImg; mImg.style.opacity = '1'; }, 100);
    }
    const mTagTitle = document.getElementById('wacMediaTagTitle');
    if(mTagTitle) mTagTitle.textContent = d.mediaTagTitle;
    const mTagSub = document.getElementById('wacMediaTagSub');
    if(mTagSub) mTagSub.textContent = d.mediaTagSub;
    const mCta = document.getElementById('wacMediaCta');
    if(mCta) mCta.textContent = d.mediaCta;
  }, 200);

  // 3. Stage 3 (t = 600ms): Send Broadcast Flash & Laser Stream
  setTimeout(()=>{
    const sendBtn = document.getElementById('wacSendBtn');
    if(sendBtn){
      const origHtml = sendBtn.innerHTML;
      sendBtn.innerHTML = '✓ Broadcast Sent!';
  ...
  // 4. Stage 4 (t = 900ms): Phone Mockup Typing & Message Pop
  setTimeout(()=>{
    setWaActiveStep(3);
    const phAv = document.getElementById('waPhAvImg');
    if(phAv) phAv.src = d.customerAvatar;
    const phName = document.getElementById('waPhName');
    if(phName) phName.textContent = d.customerName;
    ...
    setTimeout(()=>{
      ...
      // Progression: single tick -> double tick -> blue tick (Read)
      setTimeout(()=>{ if(ticks) ticks.textContent = '✓✓'; }, 400);
      setTimeout(()=>{
        if(ticks){
      ...
      setTimeout(()=>{
        ...
        if(phOutMsg) phOutMsg.innerHTML = d.phoneOutMsg;
        if(outBubble){
  ...
  // 5. Stage 5 (t = 2400ms): Analytics Draw & Rolling Numbers
  setTimeout(()=>{
    setWaActiveStep(4);
    
    // Funnel count-up
    animateCount(document.getElementById('waStatSent'), 0, d.stats.sentRaw);
    animateCount(document.getElementById('waStatDelivered'), 0, d.stats.deliveredRaw);
    animateCount(document.getElementById('waStatRead'), 0, d.stats.readRaw);
    animateCount(document.getElementById('waStatClicked'), 0, d.stats.clickedRaw);
    animateCount(document.getElementById('waStatPurchases'), 0, d.stats.purchasesRaw);

    const sDeliveredPill = document.getElementById('waStatDeliveredPill');
    if(sDeliveredPill) sDeliveredPill.textContent = d.stats.deliveredPill;
    const sReadPill = document.getElementById('waStatReadPill');
    if(sReadPill) sReadPill.textContent = d.stats.readPill;
    const sClickedPill = document.getElementById('waStatClickedPill');
    if(sClickedPill) sClickedPill.textContent = d.stats.clickedPill;

    const sConvRate = document.getElementById('waStatConvRate');
    if(sConvRate) sConvRate.textContent = d.stats.convRate;
    const sRev = document.getElementById('waStatRev');
    if(sRev) sRev.textContent = d.stats.rev;
    const sRevBadge = document.getElementById('waStatRevBadge');
    if(sRevBadge) sRevBadge.textContent = d.stats.revBadge;

    // Redraw SVG Curves
    document.querySelectorAll('#waChartSvg path.anim-curve').forEach(p=>{
      p.style.animation = 'none';
      p.offsetHeight;
      p.style.animation = 'waDrawPath 1.6s cubic-bezier(.2,.8,.3,1) both';
    });

    // Tooltip update
    const ttSent = document.getElementById('ttSent');
    if(ttSent) ttSent.textContent = d.stats.tt.sent;
    const ttDelivered = document.getElementById('ttDelivered');
    if(ttDelivered) ttDelivered.textContent = d.stats.tt.delivered;
    const ttRead = document.getElementById('ttRead');
    if(ttRead) ttRead.textContent = d.stats.tt.read;
    const ttClicked = document.getElementById('ttClicked');
    if(ttClicked) ttClicked.textContent = d.stats.tt.clicked;
    const ttPurchased = document.getElementById('ttPurchased');
    if(ttPurchased) ttPurchased.textContent = d.stats.tt.purchased;

    // Recent activity row update
    const actRowName = document.getElementById('waActRowName1');
    if(actRowName) actRowName.textContent = d.customerName;
    const actRowAv = document.getElementById('waActRowAv1');
    if(actRowAv) actRowAv.src = d.customerAvatar;
  }, 2400);
}

// Attach Audience List Click Handlers
document.querySelectorAll('#wacAudList .wac-aud-item').forEach(item => {
  const handler = () => {
    const audId = item.dataset.aud;
    if(audId !== undefined) playWaWorkflow(audId);
  };
...
// Replay Auto Demo Button
const waReplayBtn = document.getElementById('waReplayBtn');
if(waReplayBtn){
  waReplayBtn.addEventListener('click', ()=>{
    const nextIdx = (currentWaSegIdx + 1) % WA_STUDIO_DATA.length;
    playWaWorkflow(nextIdx, 'auto');
  });
}
...
if(sendBtn){
  sendBtn.addEventListener('click', () => {
    playWaWorkflow(currentWaSegIdx);
  });
}

// Add Variable Button in Composer
const addVarBtn = document.getElementById('wacAddVarBtn');
if(addVarBtn){
  addVarBtn.addEventListener('click', ()=>{
    const edBox = document.getElementById('wacEditorBox');
    if(edBox){
      edBox.innerHTML += ' <b>{{Special_Perk}}</b>';
      edBox.focus();
    }
  });
}
