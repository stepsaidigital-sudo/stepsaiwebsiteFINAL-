# blog-cart-recovery.html — UI implementation brief

Status: planned, not implemented. Apply after the shared shell and Sales Agent pilot.

## Objective and constraints

Explain cart recovery. Existing title: “Recovering Abandoned Carts on Autopilot — StepsAI Blog”. Preserve useful content, routes and actual product claims. Read [shared system](../02-SHARED-SYSTEM.md), [asset plan](../03-ASSET-AND-MOCKUP-PLAN.md) and [implementation gates](../04-IMPLEMENTATION.md) first.

## Evidence and edit boundary

- Source: [blog-cart-recovery.html](../../../blog-cart-recovery.html). Local styles in cascade order: `styles.css`, `nav-mega-product.css`, `industries-deep.css`, `agents.css`, `blog.css`.
...
- 1 dark-marked sections; 1 legacy logo reference(s) at lines 697. No favicon reference detected; add the shared approved favicon as part of shell work.
...
Source: `blog-cart-recovery.html:656`; current classes: `section section--dark final-cta`. Audit desktop height: 501px; padding: 120px 0px.
...
- `blog-cart-recovery.html:495` — Recovering Abandoned Carts on Autopilot
- `blog-cart-recovery.html:558` — The problem with manual follow-up
- `blog-cart-recovery.html:562` — The workflow, node by node
- `blog-cart-recovery.html:565` — Step 1: the trigger and the wait
- `blog-cart-recovery.html:576` — Step 2: the check that matters
- `blog-cart-recovery.html:579` — Step 3: the message, then the handoff
- `blog-cart-recovery.html:590` — Why this works better than a discount blast
- `blog-cart-recovery.html:658` — Paste your website link. Watch it start answering.

## Mockup and responsive composition

Annotated abandoned-cart message sequence; avoid implying unsupported recovery rates.
...
- Proposed candidates: Existing matching product photos.
...
1. The mockup communicates “explain cart recovery” and the original content landmarks remain accessible.
