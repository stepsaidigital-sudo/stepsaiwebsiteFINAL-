# meetings-agent.html — UI implementation brief

Status: planned, not implemented. Apply after the shared shell and Sales Agent pilot.

## Objective and constraints

Explain scheduling without repeated messages. Existing title: “Meetings Agent — StepsAI | Real slots. Real bookings. No back-and-forth.”. Preserve useful content, routes and actual product claims. Read [shared system](../02-SHARED-SYSTEM.md), [asset plan](../03-ASSET-AND-MOCKUP-PLAN.md) and [implementation gates](../04-IMPLEMENTATION.md) first.

## Evidence and edit boundary

- Source: [meetings-agent.html](../../../meetings-agent.html). Local styles in cascade order: `styles.css`, `nav-mega-product.css`, `industries-deep.css`, `agents.css`, `channel-premium.css`, `channel-premium-bold.css`, `signature-meetings.css`.
...
- 2 dark-marked sections; 1 legacy logo reference(s) at lines 775. No favicon reference detected; add the shared approved favicon as part of shell work.
...
### 1. Real slots. Real bookings. No back-and-forth.

Source: `meetings-agent.html:491`; current classes: `vertical-hero`. Audit desktop height: 618px; padding: 72px 0px 90px.
...
### 2. Booking that ends in a confirmed time, not a "let me check."

Source: `meetings-agent.html:538`; current classes: `section section--raised section--atmo`. Audit desktop height: 1691px; padding: 120px 0px.
...
Retain these content units: “It checks your actual calendar, live.”; “Confirmations and reminders happen automatically.”; “Plans change — it handles that too.”.

### 3. The conversation and the calendar are the same event.

Source: `meetings-agent.html:625`; current classes: `section section--base`. Audit desktop height: 783px; padding: 120px 0px.

Keep the content sequence and distinctive demonstration. Use the shared reading width, type scale and spacing; remove competing decorative layers. Stack copy before its visual on mobile and keep meaningful UI text readable.

### 4. Books into the calendar you already use.

Source: `meetings-agent.html:677`; current classes: `section section--dark`. Audit desktop height: 458px; padding: 120px 0px.
...
### 5. Questions about the Meetings Agent.

Source: `meetings-agent.html:696`; current classes: `section section--base section--atmo`. Audit desktop height: 631px; padding: 120px 0px.
...
### 6. Your next booking doesn't need a back-and-forth.

Source: `meetings-agent.html:734`; current classes: `section section--dark final-cta`. Audit desktop height: 501px; padding: 120px 0px.
...
- `meetings-agent.html:496` — Real slots. Real bookings. No back-and-forth.
- `meetings-agent.html:546` — Booking that ends in a confirmed time, not a "let me check."
- `meetings-agent.html:629` — The conversation and the calendar are the same event.
- `meetings-agent.html:682` — Books into the calendar you already use.
- `meetings-agent.html:704` — Questions about the Meetings Agent.
- `meetings-agent.html:736` — Your next booking doesn't need a back-and-forth.

## Mockup and responsive composition

Show availability, timezone selection, slot confirmation and calendar handoff; include an unavailable-slot state.
...
- Proposed candidates: Calendar interface built in HTML/CSS.
...
1. The mockup communicates “explain scheduling without repeated messages” and the original content landmarks remain accessible.
