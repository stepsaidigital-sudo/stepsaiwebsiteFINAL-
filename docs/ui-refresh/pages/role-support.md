# role-support.html — UI implementation brief

Status: planned, not implemented. Apply after the shared shell and Sales Agent pilot.

## Objective and constraints

Explain support operations. Existing title: “Support &mdash; StepsAI | Resolve tickets and delight customers”. Preserve useful content, routes and actual product claims. Read [shared system](../02-SHARED-SYSTEM.md), [asset plan](../03-ASSET-AND-MOCKUP-PLAN.md) and [implementation gates](../04-IMPLEMENTATION.md) first.

## Evidence and edit boundary

- Source: [role-support.html](../../../role-support.html). Local styles in cascade order: `styles.css`, `nav-mega-product.css`, `industries-deep.css`, `agents.css`.
...
- 1 dark-marked sections; 1 legacy logo reference(s) at lines 772. No favicon reference detected; add the shared approved favicon as part of shell work.
...
### 1. Most tickets are the same five questions. Let it answer those.

Source: `role-support.html:485`; current classes: `vertical-hero`. Audit desktop height: 661px; padding: 72px 0px 90px.
...
Source: `role-support.html:504`; current classes: `section section--base section--atmo`. Audit desktop height: 942px; padding: 120px 0px.
...
Source: `role-support.html:556`; current classes: `section section--base pain-use-section section--atmo`. Audit desktop height: 2422px; padding: 120px 0px.

Keep the content sequence and distinctive demonstration. Use the shared reading width, type scale and spacing; remove competing decorative layers. Stack copy before its visual on mobile and keep meaningful UI text readable.

Retain these content units: “Resolve”; “Escalate cleanly”; “Learn & report”.

### 4. What it will never do

Source: `role-support.html:609`; current classes: `section section--raised section--atmo`. Audit desktop height: 665px; padding: 120px 0px.

Keep the content sequence and distinctive demonstration. Use the shared reading width, type scale and spacing; remove competing decorative layers. Stack copy before its visual on mobile and keep meaningful UI text readable.

### 5. If you can only start with one thing

Source: `role-support.html:636`; current classes: `section section--base section--atmo`. Audit desktop height: 734px; padding: 120px 0px.

Keep the content sequence and distinctive demonstration. Use the shared reading width, type scale and spacing; remove competing decorative layers. Stack copy before its visual on mobile and keep meaningful UI text readable.

### 6. What follow-up looks like after a resolution

Source: `role-support.html:657`; current classes: `section section--raised section--atmo`. Audit desktop height: 694px; padding: 120px 0px.

Keep the content sequence and distinctive demonstration. Use the shared reading width, type scale and spacing; remove competing decorative layers. Stack copy before its visual on mobile and keep meaningful UI text readable.

Preserve destinations: `See the platform capabilities → capabilities.html`.

### 7. About the support role.

Source: `role-support.html:693`; current classes: `section section--raised section--atmo`. Audit desktop height: 631px; padding: 120px 0px.

Keep the content sequence and distinctive demonstration. Use the shared reading width, type scale and spacing; remove competing decorative layers. Stack copy before its visual on mobile and keep meaningful UI text readable.

### 8. Your queue just got five questions shorter.

Source: `role-support.html:729`; current classes: `section section--dark final-cta`. Audit desktop height: 493px; padding: 120px 0px.
...
- `role-support.html:489` — Most tickets are the same five questions. Let it answer those.
- `role-support.html:590` — You already know these
- `role-support.html:617` — What it will never do
- `role-support.html:644` — If you can only start with one thing
- `role-support.html:665` — What follow-up looks like after a resolution
- `role-support.html:701` — About the support role.
- `role-support.html:733` — Your queue just got five questions shorter.

## Mockup and responsive composition

Queue prioritization, resolved answer and escalation; distinguish automation from human ownership.
...
- Proposed candidates: Editable support queue.
...
1. The mockup communicates “explain support operations” and the original content landmarks remain accessible.
