# integrations.html — UI implementation brief

Status: planned, not implemented. Apply after the shared shell and Sales Agent pilot.

## Objective and constraints

Help find a compatible tool. Existing title: “Integrations — StepsAI | Plugs into what you already run”. Preserve useful content, routes and actual product claims. Read [shared system](../02-SHARED-SYSTEM.md), [asset plan](../03-ASSET-AND-MOCKUP-PLAN.md) and [implementation gates](../04-IMPLEMENTATION.md) first.

## Evidence and edit boundary

- Source: [integrations.html](../../../integrations.html). Local styles in cascade order: `styles.css`, `nav-mega-product.css`, `industries-deep.css`, `agents.css`, `channel-premium.css`, `channel-premium-bold.css`, `signature-integrations.css`.
...
- 2 dark-marked sections; 1 legacy logo reference(s) at lines 691. No favicon reference detected; add the shared approved favicon as part of shell work.
...
### 1. Plugs into what you already run. No rip-and-replace.

Source: `integrations.html:491`; current classes: `vertical-hero`. Audit desktop height: 512px; padding: 72px 0px 90px.
...
### 2. Every integration unlocks a real action.

Source: `integrations.html:503`; current classes: `section section--dark`. Audit desktop height: 1313px; padding: 120px 0px.
...
### 3. Connect once. It works everywhere.

Source: `integrations.html:537`; current classes: `section section--base section--atmo`. Audit desktop height: 820px; padding: 120px 0px.
...
### 4. Nothing to connect? It still works.

Source: `integrations.html:590`; current classes: `section section--raised section--atmo`. Audit desktop height: 682px; padding: 120px 0px.

Keep the content sequence and distinctive demonstration. Use the shared reading width, type scale and spacing; remove competing decorative layers. Stack copy before its visual on mobile and keep meaningful UI text readable. For provider rows use white cards on a pale-blue band, verified provider marks and readable connection labels. A static status badge must not look like an interactive Connect button.

### 5. About connecting your tools.

Source: `integrations.html:612`; current classes: `section section--base section--atmo`. Audit desktop height: 631px; padding: 120px 0px.

Keep the content sequence and distinctive demonstration. Use the shared reading width, type scale and spacing; remove competing decorative layers. Stack copy before its visual on mobile and keep meaningful UI text readable. For provider rows use white cards on a pale-blue band, verified provider marks and readable connection labels. A static status badge must not look like an interactive Connect button.

### 6. Connect what you use. Skip what you don't.

Source: `integrations.html:650`; current classes: `section section--dark final-cta`. Audit desktop height: 501px; padding: 120px 0px.
...
- `integrations.html:495` — Plugs into what you already run. No rip-and-replace.
- `integrations.html:508` — Every integration unlocks a real action.
- `integrations.html:545` — Connect once. It works everywhere.
- `integrations.html:598` — Nothing to connect? It still works.
- `integrations.html:620` — About connecting your tools.
- `integrations.html:652` — Connect what you use. Skip what you don't.

## Mockup and responsive composition

Light searchable provider directory with readable categories, provider details and no-result state; distinguish supported integrations from requests.
...
- Proposed candidates: Verified official provider logos.
- Stock decision: No stock required; never replace provider marks with unrelated icons.
...
1. The mockup communicates “help find a compatible tool” and the original content landmarks remain accessible.
