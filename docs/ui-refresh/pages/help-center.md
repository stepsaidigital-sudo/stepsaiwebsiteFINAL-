# help-center.html — UI implementation brief

Status: planned, not implemented. Apply after the shared shell and Sales Agent pilot.

## Objective and constraints

Help visitors find an answer. Existing title: “Help Center — StepsAI”. Preserve useful content, routes and actual product claims. Read [shared system](../02-SHARED-SYSTEM.md), [asset plan](../03-ASSET-AND-MOCKUP-PLAN.md) and [implementation gates](../04-IMPLEMENTATION.md) first.

## Evidence and edit boundary

- Source: [help-center.html](../../../help-center.html). Local styles in cascade order: `styles.css`, `nav-mega-product.css`, `pages.css`, `blog.css`.
...
- 1 dark-marked sections; 1 legacy logo reference(s) at lines 662. No favicon reference detected; add the shared approved favicon as part of shell work.
...
Source: `help-center.html:493`; current classes: `section section--base page-hero section--atmo`. Audit desktop height: 379px; padding: 116px 0px 44px.
...
Preserve destinations: `Contact us directly → contact.html`.

### 2. topics

Source: `help-center.html:516`; current classes: `section section--raised section--atmo`. Audit desktop height: 1607px; padding: 120px 0px.
...
Preserve destinations: `pricing → pricing.html`; `channels overview → channels.html`; `Integrations → integrations.html`; `built-in CRM → crm.html`; `pricing page → pricing.html`; `Hire an Agency → hire-an-agency.html`; `Workflows → workflows.html`; `WhatsApp Broadcast → whatsapp-broadcast.html`; `Privacy Policy → privacy-policy.html`; `Terms of Service → terms-of-service.html`; `contact page → contact.html`; `contact us directly → contact.html`.

### 3. Still stuck? A real person will answer.

Source: `help-center.html:620`; current classes: `section section--dark final-cta`. Audit desktop height: 457px; padding: 120px 0px.
...
- `help-center.html:501` — How can we help?
- `help-center.html:624` — Still stuck? A real person will answer.

## Mockup and responsive composition

Search/category/article hierarchy; show no-result and keyboard-focus states if search exists.
...
- Proposed candidates: Simple functional icons.
...
1. The mockup communicates “help visitors find an answer” and the original content landmarks remain accessible.
