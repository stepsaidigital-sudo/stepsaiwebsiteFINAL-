# crm.html — UI implementation brief

Status: planned, not implemented. Apply after the shared shell and Sales Agent pilot.

## Objective and constraints

Explain connected customer context. Existing title: “Built-in CRM & Tickets — StepsAI | Every contact, already on record”. Preserve useful content, routes and actual product claims. Read [shared system](../02-SHARED-SYSTEM.md), [asset plan](../03-ASSET-AND-MOCKUP-PLAN.md) and [implementation gates](../04-IMPLEMENTATION.md) first.

## Evidence and edit boundary

- Source: [crm.html](../../../crm.html). Local styles in cascade order: `styles.css?v=2.0`, `nav-mega-product.css`, `industries-deep.css?v=2.0`, `agents.css?v=2.0`, `channel-premium.css`, `channel-premium-bold.css`, `signature-crm.css`.
...
- 1 dark-marked sections; 1 legacy logo reference(s) at lines 779. A favicon reference exists; confirm current identity.
...
### 1. Every contact, conversation and ticket — already where you need them.

Source: `crm.html:496`; current classes: `vertical-hero`. Audit desktop height: 611px; padding: 72px 0px 90px.
...
### 2. What a contact actually looks like.

Source: `crm.html:511`; current classes: `section section--base section--atmo`. Audit desktop height: 1179px; padding: 120px 0px.
...
Retain these content units: “Not three contacts. One.”.

### 3. Automatic doesn't mean out of your hands.

Source: `crm.html:617`; current classes: `section section--raised section--atmo`. Audit desktop height: 520px; padding: 120px 0px.

Keep the content sequence and distinctive demonstration. Use the shared reading width, type scale and spacing; remove competing decorative layers. Stack copy before its visual on mobile and keep meaningful UI text readable.

### 4. Nothing lives only in someone's memory.

Source: `crm.html:641`; current classes: `section section--base section--atmo`. Audit desktop height: 633px; padding: 120px 0px.

Keep the content sequence and distinctive demonstration. Use the shared reading width, type scale and spacing; remove competing decorative layers. Stack copy before its visual on mobile and keep meaningful UI text readable.

### 5. About the CRM.

Source: `crm.html:662`; current classes: `section section--raised section--atmo`. Audit desktop height: 767px; padding: 120px 0px.

Keep the content sequence and distinctive demonstration. Use the shared reading width, type scale and spacing; remove competing decorative layers. Stack copy before its visual on mobile and keep meaningful UI text readable.

### 6. Never ask "wait, who is this?" again.

Source: `crm.html:738`; current classes: `section section--dark final-cta`. Audit desktop height: 501px; padding: 120px 0px.
...
- `crm.html:500` — Every contact, conversation and ticket — already where you need them.
- `crm.html:519` — What a contact actually looks like.
- `crm.html:625` — Automatic doesn't mean out of your hands.
- `crm.html:649` — Nothing lives only in someone's memory.
- `crm.html:670` — About the CRM.
- `crm.html:740` — Never ask "wait, who is this?" again.

## Mockup and responsive composition

Show contact identity, conversation history and next action with fictional data and consent-safe labels.
...
- Proposed candidates: Editable contact record.
...
1. The mockup communicates “explain connected customer context” and the original content landmarks remain accessible.
