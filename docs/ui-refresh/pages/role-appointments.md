# role-appointments.html — UI implementation brief

Status: planned, not implemented. Apply after the shared shell and Sales Agent pilot.

## Objective and constraints

Explain booking operations. Existing title: “Appointments &mdash; StepsAI | Scheduling, reminders and the follow-through”. Preserve useful content, routes and actual product claims. Read [shared system](../02-SHARED-SYSTEM.md), [asset plan](../03-ASSET-AND-MOCKUP-PLAN.md) and [implementation gates](../04-IMPLEMENTATION.md) first.

## Evidence and edit boundary

- Source: [role-appointments.html](../../../role-appointments.html). Local styles in cascade order: `styles.css`, `nav-mega-product.css`, `industries-deep.css`, `agents.css`.
...
- 2 dark-marked sections; 1 legacy logo reference(s) at lines 802. No favicon reference detected; add the shared approved favicon as part of shell work.
...
### 1. The booking is the easy part. The follow-through is where it usually drops.

Source: `role-appointments.html:485`; current classes: `vertical-hero`. Audit desktop height: 661px; padding: 72px 0px 90px.
...
Source: `role-appointments.html:504`; current classes: `section section--base section--atmo`. Audit desktop height: 942px; padding: 120px 0px.
...
Source: `role-appointments.html:564`; current classes: `section section--base pain-use-section section--atmo`. Audit desktop height: 2401px; padding: 120px 0px.

Keep the content sequence and distinctive demonstration. Use the shared reading width, type scale and spacing; remove competing decorative layers. Stack copy before its visual on mobile and keep meaningful UI text readable.

Retain these content units: “Book & remind”; “Recover & automate”; “Report”.

### 4. What it will never do

Source: `role-appointments.html:617`; current classes: `section section--raised section--atmo`. Audit desktop height: 665px; padding: 120px 0px.

Keep the content sequence and distinctive demonstration. Use the shared reading width, type scale and spacing; remove competing decorative layers. Stack copy before its visual on mobile and keep meaningful UI text readable.

### 5. If you can only start with one thing

Source: `role-appointments.html:644`; current classes: `section section--base section--atmo`. Audit desktop height: 707px; padding: 120px 0px.

Keep the content sequence and distinctive demonstration. Use the shared reading width, type scale and spacing; remove competing decorative layers. Stack copy before its visual on mobile and keep meaningful UI text readable.

### 6. What follow-up looks like after the appointment

Source: `role-appointments.html:665`; current classes: `section section--raised section--atmo`. Audit desktop height: 715px; padding: 120px 0px.

Keep the content sequence and distinctive demonstration. Use the shared reading width, type scale and spacing; remove competing decorative layers. Stack copy before its visual on mobile and keep meaningful UI text readable.

Preserve destinations: `See the platform capabilities → capabilities.html`.

### 7. About the appointments role.

Source: `role-appointments.html:701`; current classes: `section section--raised section--atmo`. Audit desktop height: 631px; padding: 120px 0px.

Keep the content sequence and distinctive demonstration. Use the shared reading width, type scale and spacing; remove competing decorative layers. Stack copy before its visual on mobile and keep meaningful UI text readable.

### 8. No calendar juggling. It books straight into what you use.

Source: `role-appointments.html:741`; current classes: `section section--dark`. Audit desktop height: 481px; padding: 120px 0px.
...
### 9. Someone just tried to book, reschedule, or cancel.

Source: `role-appointments.html:759`; current classes: `section section--dark final-cta`. Audit desktop height: 493px; padding: 120px 0px.
...
- `role-appointments.html:489` — The booking is the easy part. The follow-through is where it usually drops.
- `role-appointments.html:598` — You already know these
- `role-appointments.html:625` — What it will never do
- `role-appointments.html:652` — If you can only start with one thing
- `role-appointments.html:673` — What follow-up looks like after the appointment
- `role-appointments.html:709` — About the appointments role.
- `role-appointments.html:746` — No calendar juggling. It books straight into what you use.
- `role-appointments.html:763` — Someone just tried to book, reschedule, or cancel.

## Mockup and responsive composition

Enquiry to scheduled appointment with timezone and unavailable-slot states.
...
- Proposed candidates: Editable scheduling UI.
...
1. The mockup communicates “explain booking operations” and the original content landmarks remain accessible.
